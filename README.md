...
- **`application.html`** — the printable Day-1 "Good Conduct Certificate Application" (A4, English) the applicant verifies, with an **Approve Application** action for authorized officers. The police emblem (`images/police_logo.png`) appears **only as the centred, low-opacity watermark** behind the body content (opacity 0.10, `mix-blend-mode: multiply` so it always reads on white paper) — the masthead is text-only. Official pre-printed-form layout: bold label + officer entry on one ruled line, mapping **every** registration field 1:1 — Section 01 carries all applicant identity fields (4-part legal name, mother's name, National ID, passport, DOB, place of birth, gender, occupation, phone, email, residence + the 35×45 mm photo box), Section 02 the purpose **selector chips**, supporting-document notes and applicant/guardian attachment checklists, Section 03 all six guardian/guarantor fields, plus **serial number / QR verification placeholders**. All web chrome (toolbar, buttons, badges, review-lock banner, toast) is hidden when printing.
- **`certificate.html`** — the official Day-2 **Certificate of Good Conduct**, rebuilt as a formal certificate: ornate double frame (navy outer + gold inner rule with corner marks), the crest in the top masthead **and** the centred watermark (opacity 0.09, multiply), gold-ruled title band, certification prose with the applicant's name on a gold underline, an elegant two-column details panel (not a plain table), a gold issue ribbon with the certificate number, and dual signature blocks flanking the circular "POLICE CLEARED" seal. It is **locked until an officer approves** the application, then unlocks for printing.
- **A4 print contract (both documents).** `@media print { @page { size: A4 portrait; margin: 8mm; } print-color-adjust: exact; }` — one page, exact brand colors (navy/gold/green bars, chips, seal, watermark survive printing), no second-page spillover (the on-screen sheet's `min-height`/margins are dropped on paper). `backend/test_print_fit.py` pins the rule, the emblem placement (opacity band 0.08–0.12 + multiply), every DOM binding/endpoint, and a worst-case single-page height budget computed from the stylesheet (application 262.6mm, certificate 267.2mm vs 281mm usable).
