...
- **A4 print contract (both documents).** Blueprint `@page` rules (`application`: `size:A4; margin:0` with `.page{padding:6mm 8mm 4mm 8mm}` on paper; `certificate`: `size: A4 portrait; margin: 0`), plus `print-color-adjust: exact` and all web chrome (toolbar, buttons, badges, banners, toasts) hidden when printing. The application sheet drops `min-height` on paper (the single-sheet guarantee — a 297mm-tall box with zero `@page` margin spills a blank second page on engine rounding) and the certificate scales its typography to fill the page with the signature block centered above the footer bar. `backend/test_print_fit.py` pins the rules, the emblem placement (opacity band 0.08–0.12), blueprint wording verbatim (Somali + Arabic), every DOM binding/endpoint/lock hook, the transparent RGBA seal asset, and a worst-case single-page height budget computed from the stylesheet (application 261.9mm, certificate 261.1mm vs 297mm usable).
