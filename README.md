...
- **Storage** — the **Police Cars** register still lives in the frontend local `db` (localStorage) until its server endpoint is added; **Stations** and **Officers** are now backed by SQLite tables (`police_stations`, `officers`) and sync from `GET /api/stations` / `GET /api/officers` in `syncServer()` (with a localStorage fallback for the offline demo). Central Police Search filters all three registers through the same cascading location filter — for officers it matches the officer's stored **origin** (`officerLocation()`). The `policesearch` / `stations` / `officers` / `cars` module keys are part of `ROLE_MODULES` (policesearch granted to the roles that can see the person registry; the three registration modules admin-only), so `/api/me` drives both nav visibility and the `go()` RBAC redirect.
