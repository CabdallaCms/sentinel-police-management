...
- **A4 print contract (both documents).** Blueprint `@page` rules (`application`: `size:A4; margin:0` with `.page{padding:6mm 8mm 4mm 8mm}` on paper; `certificate`: `size: A4 portrait; margin: 0`), plus `print-color-adjust: exact` and all web chrome (toolbar, buttons, badges, banners, toasts) hidden when printing. `backend/test_print_fit.py` pins the rules, the emblem placement (opacity band 0.08–0.12), blueprint wording verbatim (Somali + Arabic), every DOM binding/endpoint/lock hook, and a worst-case single-page height budget computed from the stylesheet (application 284.7mm, certificate 229.2mm vs 297mm usable).
