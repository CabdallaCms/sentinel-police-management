...
- **Station anchor model** — Police **Station Registration** captures *Station Name, Code, Region, District, Village/Town* (`requireLocation()` enforces all three levels). **Police Officer** and **Police Car** registration link a record to an assigned station; picking a station **pre-fills** the Region/District/Village fields with the station's location as editable defaults (`wireStationLocationDefaults()` → `paintLocationDefaults()`), and the officer may change or override any of the three. The values actually entered are **stored explicitly on the record** (`region`/`district`/`village` fields, required at save via `requireLocation()`), and the Registered Officers / Registered Cars tables display those stored values (`recordLocation()` + `locationCells()`), falling back to the station's location only for legacy rows saved before per-record locations existed. Central Police Search filters on the records' stored locations and matches the manually typed Village/Town level case-insensitively.
