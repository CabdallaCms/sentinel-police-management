...
- `GET /api/stations` / `POST /api/stations` (SystemAdmin only — Police Stations register; `POST` validates name/code/region/district, auto-generates `ST-XXX`, and rejects duplicate station codes)
- `GET /api/officers` / `POST /api/officers` (SystemAdmin only — Police Officers register; `POST` accepts `multipart/form-data` with the five-section officer fields plus the `photo` (required), `guarantor_photo`, `doc1_file` (required) and `doc2_file` uploads. Auto-generates `POL-YYYY-XXXX` Service IDs, enforces the rank/unit/duty/blood/relationship/document dropdown lists, the station foreign key, and the JPEG/PNG + 5 MB upload policy. Mandatory: officer photo and Document Slot 1.)
