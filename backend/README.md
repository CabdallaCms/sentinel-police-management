...
- `PATCH /api/persons/{person_id}` (authenticated — **dual-tier**: frontline officers may update only the volatile contact fields `phone`, `residence`, `occupation`, `photo_path`; changing a core identity field — name parts / full name, `date_of_birth`, `mother_name`, `national_id`, `passport_id`, `place_of_birth` — requires SystemAdmin and is rejected with 403 otherwise)
- `POST /api/persons/{person_id}/photo` (authenticated, `multipart/form-data` with a `photo` file — updates the profile photo; a volatile field, so frontline officers may use it too)
